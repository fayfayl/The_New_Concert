...
The province layer is composited with a **per-pixel alpha** rather than one opacity for the whole layer. Interiors are drawn at `SATELLITE_FILL` so the terrain reads through, while borders and coastlines stay near-solid at `SATELLITE_EDGE`, and ocean is fully transparent so the imagery's own sea shows. Fading the layer uniformly would wash the borders out along with everything else, and the borders are the part that has to stay legible.
